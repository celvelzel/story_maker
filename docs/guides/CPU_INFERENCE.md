...
> **注意**: 本文档描述的是基于 vLLM 的 CPU 推理方案。项目当前默认使用 llama.cpp 作为本地推理后端，本文档保留作为历史参考和备选方案。
...
项目当前推荐使用 **llama.cpp** 作为本地推理后端，原因：
- 更简单的部署流程
- 更好的 CPU/Metal/CUDA 支持
- 更低的内存占用
- 开箱即用的 OpenAI 兼容 API

如需使用 llama.cpp，请参考 [本地模型启动指南](local-model-startup.md) 和 [从零部署指南](zero-to-hero-deployment.md)。
