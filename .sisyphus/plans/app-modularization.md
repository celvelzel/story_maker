...
## TODOs
- [ ] [app.py: Move theme to `src/ui/layout/theme.py`] Extract 700+ lines of CSS to `load_theme()` — expect `app.py` to become significantly smaller.
- [ ] [app.py: Move state initialization to `src/ui/state_manager.py`] Create `initialize_state()` and move `_DEFAULTS` and `_restore_runtime_session_once` logic — expect cleaner setup phase.
