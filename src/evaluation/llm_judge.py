...
Rate the session on these eight dimensions (1-10):
...
6. option_relevance – how well offered options align with current context and goals
7. causal_link – whether player actions cause believable world/story state changes
8. local_coherence – continuity between adjacent turns (no abrupt local contradictions)
...
DIMENSIONS = [
    ...
    "creativity",         # 创意
    "pacing",             # 节奏
    "option_relevance",   # 选项相关性
    "causal_link",        # 行动-状态因果链
    "local_coherence",    # 局部连贯性
]


def _extract_json_object(raw: object) -> Dict[str, object]:
    """Parse model response into a dict with robust fallbacks."""
    ...
    # Fallback: try to extract the outermost JSON object slice.
    start = text.find("{")
    end = text.rfind("}")
...
def _normalize_score(raw_value: object) -> int:
    """Coerce arbitrary score values into [1, 10] integer range."""
    try:
